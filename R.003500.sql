...
Name: GHW - Local checkouts and renewals last month - still in testing
...
Modified on: 2021-05-03 01:08:45
Date last run: 2021-05-07 16:44:54
...
FROM
  (SELECT
   ...
   FROM
     ...
     itemtypes
   WHERE
     branches.branchcode LIKE <<Choose your library|LBRANCH>>) branch_itype LEFT JOIN
  (SELECT
   ...
   WHERE
     ...
     If(
       ...
       If(
         Coalesce(statistics.branch, 'NEKLS') LIKE 'PH%', 'PH%', 
         statistics.branch
       )
     ) = If(
       Coalesce(items.homebranch, 'NEKLS') LIKE 'DONI%', 'DONI%', 
       If(
         Coalesce(items.homebranch, 'NEKLS') LIKE 'PH%', 'PH%', 
         items.homebranch
       )
     ) AND
