...
Modified on: 2020-06-17 11:27:55
Date last run: 2020-06-29 15:50:51
...
SELECT
  ...
  Concat_Ws('<br />', If(name.value = 1, Concat(borrowers.surname, ', ',
  borrowers.firstname, If(borrowers.othernames = '', '', Concat(' (',
  borrowers.othernames, ')'))),
  '<u>Patron does not want their name shared with other members</u><br />'),
  If(email.value = 1, borrowers.email,
  '<u>Patron does not want their e-mail shared with other members</u><br />'),
  If(phone.value = 1, borrowers.phone,
  '<u>Patron does not want their phone number shared with other members</u><br />'), Concat('Joined on: ', Month(club_enrollments.date_enrolled), '/', Day(club_enrollments.date_enrolled), '/', Year(club_enrollments.date_enrolled))) AS CLUB_ROSTER_FOR_CLUB_MEMBERS
FROM
  clubs JOIN
