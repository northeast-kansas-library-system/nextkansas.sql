...
SELECT CONCAT (''"link to record"'') AS "link to record", b.title, (SELECT count(r.reserve_id) FROM reserves r WHERE r.biblionumber=b.biblionumber AND r.branchcode LIKE 'DONI%') as your_patron_holds, (SELECT i.ccode FROM items i WHERE i.biblionumber=r.biblionumber LIMIT 1) as collection, (SELECT count(i.itemnumber) FROM items i WHERE i.biblionumber=r.biblionumber AND i.damaged='0' AND i.withdrawn='0' AND itemlost='0' AND i.homebranch LIKE 'DONI%') as your_lib_holdings FROM biblio b LEFT JOIN reserves r USING (biblionumber) LEFT JOIN items i USING(biblionumber) WHERE r.biblionumber=b.biblionumber AND r.branchcode LIKE 'DONI%' AND r.biblionumber IN (SELECT r.biblionumber FROM reserves r WHERE r.biblionumber=b.biblionumber AND r.branchcode LIKE 'DONI%' GROUP BY r.biblionumber HAVING count(r.reserve_id) &gt; '2') GROUP BY b.biblionumber HAVING count(r.reserve_id) &gt; '2' ORDER BY your_lib_holdings asc
