...
WHERE 
  ...
  (branches.branchcode LIKE @brn:=<<Enter check out branch|LBRANCH>> COLLATE utf8mb4_unicode_ci) AND 
  (items.homebranch NOT LIKE @brn) AND 
  items.permanent_location LIKE <<Choose a shelving location|LLOC>> AND 
  statistics.itemtype LIKE <<Choose an item type|LITYPES>> AND 
  statistics.ccode LIKE <<Choose an item Collection code|LCCODE>> AND 
  statistics.datetime BETWEEN <<Checked out between the start of the day on date1|date>> and (<<and the end of the day on date2|date>> + interval 1 day) 
