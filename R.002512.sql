...
Name: Call Number Monthly Circ Report for Basehor items, checked out at Basehor
...
Created on: 2015-05-26 10:17:58
Modified on: 2015-05-26 10:25:17
Date last run: -
...
Returns total circ for any given month, based on call number breakdown (Basehor items checked out anywhere). You'll need to plug in the month (ex. 02) and year (ex. 2015) for the month you want to look at circulations; you'll also need to plug in that month's last day (ex. 2015-04-30) so the report only looks at items added through that month. <b><a href="https://staff.nexpresslibrary.org/cgi-bin/koha/reports/guided_reports.pl?reports=2500&phase=Run%20this%20report" target="_blank">Run report #2500</a></b> to get total items for this grouping of call numbers.
...
WHERE i.homebranch='BASEHOR' AND month(s.datetime)=<<enter two-digit month, like 05 for May>> AND year(s.datetime)=<<enter four-digit year, like 2015>> AND s.type IN ('issue','renew') AND s.branch='BASEHOR' AND i.dateaccessioned <= <<enter last date of month you're counting stats on, like 2015-04-30>>
