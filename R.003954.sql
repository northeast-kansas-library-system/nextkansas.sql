...
SELECT
  ...
  CASE
    WHEN
      rules_table.daysmode = 0
    THEN 'Default'
    WHEN
      rules_table.daysmode IS NULL
    THEN 'Default'
    WHEN
      rules_table.daysmode = ''
    THEN 'Default'
    WHEN
      rules_table.daysmode = 'Calendar'
    THEN 'Skip'
    WHEN
      rules_table.daysmode = 'Datedue'
    THEN 'Next open day'
    WHEN
      rules_table.daysmode = 'Days'
    THEN 'Ignore the calendar'
    WHEN
      rules_table.daysmode = 'Dayweek'
    THEN 'Same week day'
  ...
  CASE
    WHEN
      rules_table.hardduedatecompare = -1
    THEN 'Before'
    WHEN
      rules_table.hardduedatecompare = 0
    THEN 'After'
    WHEN
      rules_table.hardduedatecompare = 1
    THEN 'Exactly on'
  ...
  CASE
    WHEN
      rules_table.chargeperiod_charge_at = 0
    THEN 'End of interval'
    WHEN
      rules_table.chargeperiod_charge_at = 1
  ...
  rules_table.no_auto_renewal_after_hard_limit AS
  ...
  CASE
    WHEN
      rules_table.opacitemholds = 'N'
    THEN "Don't allow"
    WHEN
      rules_table.opacitemholds = 'Y'
    THEN 'Allow'
    WHEN
      rules_table.opacitemholds = 'F'
    THEN 'Force'
  ...
  (SELECT
    ...
    FROM
      ...
      (SELECT
        ...
        FROM
          itemtypes) itypess) branches_categories_itypes JOIN
  (SELECT
      ...
      Max(CASE
        WHEN
          circulation_rules.rule_name = 'expire_reserves_charge'
        THEN circulation_rules.rule_value
        ELSE NULL
    ...
    HAVING
      lengthunit IS NOT NULL) rules_table ON rules_table.branch =
      branches_categories_itypes.branchcode AND
      rules_table.cat = branches_categories_itypes.categorycode AND
      rules_table.type = branches_categories_itypes.itemtype,
  (SELECT
      Coalesce(Max(CASE
        WHEN
          circulation_rules.rule_name = 'maxissueqty' AND Concat_Ws('_',
          circulation_rules.branchcode, Coalesce(circulation_rules.categorycode,
          'all'), Coalesce(circulation_rules.itemtype, 'all')) =
...
WHERE
  (branches_categories_itypes.branchcode LIKE <<Your library|branches:all>> OR
    branches_categories_itypes.branchcode LIKE CASE
      WHEN
        rule_check.All_Rule = 0
      THEN 'all'
      ELSE 'x'
    END)
ORDER BY
  If(branches_categories_itypes.branchname = ' All libraries', 'Z',
  branches_categories_itypes.branchname),
  If(branches_categories_itypes.description = ' All categories', 'Z',
  branches_categories_itypes.description),
  If(branches_categories_itypes.description1 = ' All item types', 'Z',
