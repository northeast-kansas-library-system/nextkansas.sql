...
SELECT
  ...
  If(
    ...
    '<br><span class="results_summary suppressed_opac next_label next_red next_embiggen3">This title is not visible to patrons in Aspen Discovery</span>', 
    ''
  ) AS SUPPRESSED
FROM
  biblio_metadata
WHERE
  biblio_metadata.biblionumber = <<Enter biblionumber>>
ORDER BY
