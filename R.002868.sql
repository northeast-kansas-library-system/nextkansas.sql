...
Modified on: 2019-01-01 14:25:48
Date last run: 2019-01-01 14:25:54
...
SELECT
  ...
  (Coalesce(ALL_ITEMS.COUNT, 0) - (Coalesce(ALL_BOOKS.Count_itemnumber, 0) + Coalesce(AUDIO_T.COUNT, 0) + (Coalesce(VIDEO_T.COUNT, 0)))) AS '9.5_OTHER_T'
FROM
  branches
  LEFT JOIN (SELECT
      items.homebranch,
    ...
    GROUP BY
      deleteditems.homebranch,
      deleteditems.itemnumber) BOOKS_A ON branches.branchcode = BOOKS_A.homebranch
  LEFT JOIN (SELECT
      deleteditems.homebranch,
    ...
    WHERE
      ...
      (deleteditems.ccode = 'BILINGUAL' OR
        ...
        deleteditems.ccode = 'WESTERN'))
    GROUP BY
      deleteditems.homebranch) BOOKS_D ON branches.branchcode = BOOKS_D.homebranch
  LEFT JOIN (SELECT
      items.homebranch,
      COUNT(*) AS COUNT
    FROM
