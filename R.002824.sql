...
Modified on: 2019-02-25 14:26:52
Date last run: 2019-08-26 11:41:56
...
SELECT
  ...
  Str_To_Date(Concat(special_holidays.month, '/', special_holidays.day, '/', special_holidays.year), '%m/%d/%Y') AS UPCOMING_DATE,
  Date_Format(Str_To_Date(Concat(special_holidays.month, '/', special_holidays.day, '/', special_holidays.year), '%m/%d/%Y'), '%W') AS DAY_OF_WEEK,
...
WHERE
  ...
  Str_To_Date(Concat(special_holidays.month, '/', special_holidays.day, '/', special_holidays.year), '%m/%d/%Y') > Now() - INTERVAL 1 DAY
UNION
SELECT
...
WHERE
  ...
  Str_To_Date(Concat(repeatable_holidays.month, '/', repeatable_holidays.day, '/', (If(repeatable_holidays.month >= Month(CurDate()), Year(CurDate()), Year(CurDate()) + 1))), '%m/%d/%Y') > Now() - INTERVAL 1 DAY) OR
  (repeatable_holidays.branchcode LIKE @brn AND Str_To_Date(Concat(repeatable_holidays.month, '/', repeatable_holidays.day, '/', (If(repeatable_holidays.month >= Month(CurDate()), Year(CurDate()), Year(CurDate()) + 1))), '%m/%d/%Y') IS NULL)
ORDER BY
  BRANCH,
  UPCOMING_DATE
