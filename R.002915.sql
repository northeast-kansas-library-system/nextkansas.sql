...
Modified on: 2021-03-31 22:54:09
Date last run: 2021-04-02 10:24:27
...
FROM 
  ...
  (SELECT 
   ...
   FROM 
     authorised_values 
   WHERE 
     authorised_values.category = 'WITHDRAWN') withdrawn ON items.withdrawn = 
      withdrawn.authorised_value LEFT JOIN 
  (SELECT 
     authorised_values.authorised_value, 
     authorised_values.lib, 
     authorised_values.category, 
  ...
  (SELECT 
   ...
   FROM 
     authorised_values 
   WHERE 
     authorised_values.category = 'LOC') permlocs ON permlocs.authorised_value = 
...
WHERE 
  ...
  lost.lib_opac LIKE <<Lost status|ZLOST>> AND 
